...
export default class {
  ...
  constructor(rows, columns) {
    ...
    if (arguments.length === 2) {
      ...
      //third argument should be the initial cells to select
      this._rows = rows;
      this._columns = columns;
      this._resetSelectionMembers();
      this._resetSelectedCells();

      let initialSelection = arguments[2];

        if (initialSelection.length !== rows) {
          return;
        }

        let rowsAreCorrectLength = true;
        if (initialSelection) {
        //  return;
        }

        this._cellsSelected = initialSelection;
      }
  }
  ...
  getCellsBeingSelected() {
    ...
    for (let i = 0; i < this._rows; i++) {
      ...
      for (let j = 0; j < this._columns; j++) {
        cells[i][j] = this._isCellBeingSelected(i, j) && (typeof this._addMode !== 'undefined') && this._addMode;
      }
    }
    return cells;
  }
  ...
  getQuarter(row) {
    ...
    return 'quarter' + row % 4;
  }

  // Is the cell at the given location currently under a selection?
